    ...
    public class CrossPromoManager : MonoBehaviour
    {
        ...
        public void Initialize(UnityAction<bool, string> CompleteMethod = null)
        {
            if (initialized == false)
            {
                ...
                CrossPromoData crossPromoSettings = Resources.Load<CrossPromoData>(Constants.DATA_NAME_RUNTIME);
                if (crossPromoSettings == null)
                {
                    Debug.LogWarning("Gley Cross Promo Plugin is not properly configured. Go to Tools->Gley->Cross Promo to set up the plugin. See the documentation");
                    return;
                }
        ...
        internal void CrossPromoClosed(bool clicked, string imageName)
        {
            if (PopupClosed != null)
            {
