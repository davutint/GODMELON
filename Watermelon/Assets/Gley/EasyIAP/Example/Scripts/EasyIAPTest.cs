...
{
    public class EasyIAPTest : MonoBehaviour
    {
                    ...
                    if (purchaseInProgress == false)
                    {
                        ...
                        if (consumableProducts.Count > 0)
                        {
                            ...
                            if (Button("BUY " + consumableProducts[indexNumberConsumable].name + " " + Gley.EasyIAP.API.GetValue(consumableProducts[indexNumberConsumable].name) + " Coins " + Gley.EasyIAP.API.GetLocalizedPriceString(consumableProducts[indexNumberConsumable].name)))
                            {
                                ...
                                Gley.EasyIAP.API.BuyProduct(consumableProducts[indexNumberConsumable].name, ProductBought);
                            }
                        }
                        else
                        {
                        ...
                        if (nonCOnsumableProducts.Count > 0)
                        {
                            ...
                            if (Button("BUY " + nonCOnsumableProducts[indexNumberNonConsumable].name + " " + Gley.EasyIAP.API.GetLocalizedPriceString(nonCOnsumableProducts[indexNumberNonConsumable].name)))
                            {
                                purchaseInProgress = true;

                                //this function can be called like this
                                //IAPManager.Instance.BuyProduct(ShopProductNames.NonConsumable1, ProductBought);
                                Gley.EasyIAP.API.BuyProduct(nonCOnsumableProducts[indexNumberNonConsumable].name, ProductBought);
                            }
                        }
                        else
                        {
                        ...
                        if (subscriptions.Count > 0)
                        {
                            ...
                            if (Button("BUY " + subscriptions[indexNumberSubscription].name + " " + Gley.EasyIAP.API.GetLocalizedPriceString(subscriptions[indexNumberSubscription].name)))
                            {
                                purchaseInProgress = true;

                                //this function can be called like this
                                //IAPManager.Instance.BuyProduct(ShopProductNames.Subscription1, ProductBought);
                                Gley.EasyIAP.API.BuyProduct(subscriptions[indexNumberSubscription].name, ProductBought);
                            }
                        }
                        else
                        {
        ...
        private void RestoreDone()
        {
            if (IAPManager.Instance.debug)
            {
                ...
                ScreenWriter.Write("Restore done");
        ...
        private void InitializeResult(IAPOperationStatus status, string message, List<StoreProduct> shopProducts)
        {
                ...
                for (int i = 0; i < shopProducts.Count; i++)
                {
                    ...
                    switch (shopProducts[i].productType)
                    {
                        case ProductType.Consumable:
                            consumableProducts.Add(new MyStoreProducts(Gley.EasyIAP.API.ConvertNameToShopProduct(shopProducts[i].productName), shopProducts[i].active));
                            break;
                        case ProductType.NonConsumable:
                            nonCOnsumableProducts.Add(new MyStoreProducts(Gley.EasyIAP.API.ConvertNameToShopProduct(shopProducts[i].productName), shopProducts[i].active));
                            break;
                        case ProductType.Subscription:
                            subscriptions.Add(new MyStoreProducts(Gley.EasyIAP.API.ConvertNameToShopProduct(shopProducts[i].productName), shopProducts[i].active));
                            break;
                    }
                }
            ...
            if (IAPManager.Instance.debug)
            {
                ...
                ScreenWriter.Write("Init status: " + status + " message " + message);
            }
        }
