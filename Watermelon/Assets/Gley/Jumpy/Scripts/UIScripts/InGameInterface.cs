...
namespace Gley.Jumpy
{
    ...
    public class InGameInterface : GenericInterfaceController
    {
        ...
        {
            livesText.text = GameStatus.GetLives().ToString();
        }
        ...
        {
#if GLEY_JUMPY
            tapText = Gley.Localization.API.GetText(WordIDs.TapID);
            Debug.Log(tapText);
#endif
        ...
        internal void UpdateDistance(int distance)
        {
            ...
            {
                Gley.GameServices.API.SubmitAchievement(AchievementNames.MediumJumper);
            }

            if (currentDistance >= 150)
            {
                Gley.GameServices.API.SubmitAchievement(AchievementNames.HighJumper);
            }

            if (currentDistance >= 500)
            {
                Gley.GameServices.API.SubmitAchievement(AchievementNames.KingOfJumps);
            }

            if (currentDistance >= 1000)
            {
                Gley.GameServices.API.SubmitAchievement(AchievementNames.Freedom);
            }
#endif
        }
